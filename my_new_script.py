...
# ১. ডোরেমন সেকশন (আপনার দেওয়া টেস্ট লিংকটি ১ নম্বরে সেট করা হয়েছে)
DORAEMON_PLAYLIST = [
...
def get_master_playlist():
    """খালি ঘরগুলো বাদ দিয়ে শুধুমাত্র আসল লিংকগুলোর একটি প্লেলিস্ট তৈরি করবে"""
    master_list = []
...
def create_m3u_playlist(playlist):
    """নতুন ফোল্ডার/ফাইলে .m3u প্লেলিস্ট তৈরি করার ফাংশন"""
    filename = "new_cartoon_playlist.m3u"
    try:
        with open(filename, "w", encoding="utf-8") as f:
            f.write("#EXTM3U\n")
            for index, (video_url, show_name) in enumerate(playlist, start=1):
                f.write(f'#EXTINF:-1 tvg-name="{show_name}_{index}" group-title="{show_name}", {show_name} Ep {index}\n')
                f.write(f"{video_url}\n")
        print(f"✅ সফলভাবে নতুন প্লেলিস্ট ফাইল তৈরি হয়েছে: {filename}")
    except Exception as e:
        print(f"❌ প্লেলিস্ট ফাইল তৈরি করতে সমস্যা হয়েছে: {e}")
...
def start_streaming():
    ...
    while True:
        ...
        if not playlist:
            print("⚠️ প্লেলিস্টে কোনো ভিডিও লিংক পাওয়া যায়নি! অনুগ্রহ করে কোডে লিংক বসান।")
            time.sleep(10)
            continue
            
        # গিটহাবে নতুন প্লেলিস্ট ফাইল (.m3u) তৈরি বা আপডেট করার জন্য কল করা হলো
        create_m3u_playlist(playlist)
        ...
        for index, (video_url, show_name) in enumerate(playlist, start=1):
            ...
            try:
                ...
                process.wait()  # ভিডিওটি শেষ হওয়া পর্যন্ত অপেক্ষা করবে
            except Exception as e:
                print(f"❌ এই লিংকটি চালাতে সমস্যা হয়েছে: {e}")
                time.sleep(5)
                
        print("\n🔄 প্লেলিস্টের শেষ ভিডিও শেষ হয়েছে। লুপ সিস্টেমের কারণে আবার ১ম ভিডিও থেকে শুরু হচ্ছে...")
        time.sleep(2)
